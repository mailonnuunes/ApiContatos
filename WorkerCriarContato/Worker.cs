        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                while (!stoppingToken.IsCancellationRequested)
                {
                    ...
                    channel.QueueDeclare(queue: "fila-criar-contato",
                                     durable: false,
                                     exclusive: false,
                                     autoDelete: false,
                                     arguments: null);
                    ...
                    consumer.Received += (sender, eventArgs) =>
                    {
                        ...
                        using (var scope = _scopeFactory.CreateScope())
                        {
                            ...
                            var newContactDto = JsonSerializer.Deserialize<Contact>(message);
                            contactService.CreateContact(newContactDto);
                        }
